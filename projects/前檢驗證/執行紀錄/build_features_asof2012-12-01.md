# build_features 執行紀錄｜前檢驗證｜as_of = 2012-12-01

- 產出：`E:\Projects\行銷分析\projects\前檢驗證\顧客特徵表\feat_customer_asof2012-12-01.parquet`（100 位顧客、42 欄）
- 列數流：原始交易 7,764 → 去重後 5,294 → 間隔數 5,194 → 顧客數 100

## M1 → M2 放行閘門（04 §一 步驟⑤）

- M1 閘門通過：E:\Projects\行銷分析\projects\前檢驗證\統計表\資料體檢\M1_品質檢查三桶.json 的 error 桶為空（產出於 2026-07-27T23:19:42）

## CRI 先驗群血緣（07 §3.1）

- `prior_group_type` = **none**（CRI 不得進 M6 分群矩陣）
- 先驗群欄位：（無先驗分群欄位）
- 命中人口變數清單：（無）
- 維度來源：（無）｜群數：0
